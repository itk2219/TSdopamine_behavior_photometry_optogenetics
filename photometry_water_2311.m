...
% cd('/Users/mitsukouchida/Dropbox (Uchida Lab)/Iku/WaterTonePuff');
%cd('G:\2017-@Harvard\Behavioral paradigm\DA sensor\MATLAB\Data');
cd('E:\2024-Hokudai\behavior\headfix\data\labview');

%filename = dir('IK*_US*_Analog');  % when run multiple sessions
%filename = dir('IK3482A_WM3_Analog'); 
filename = dir('IK220_waterOdor_250527');  
...
all_trial = trial_type_sig_on(:,1);
    ...
    baseline_red = tdTom(plotind); 
    F_each_red = mean(baseline_red,2);
    F_mean_red = mean(F_each_red);
...
%trial_number = [length(trial_type_ts{1}),length(trial_type_ts{2}),length(trial_type_ts{3}),...
    %length(trial_type_ts{5}),length(trial_type_ts{7}),length(trial_type_ts{9})]
% trial_number = [length(trial_type_ts{1}),length(trial_type_ts{2}),length(trial_type_ts{3}),...
...
% trigger = {trial_type_ts{1};trial_type_ts{2};trial_type_ts{3};trial_type_ts{4};...
     % trial_type_ts{5};trial_type_ts{7};trial_type_ts{9}};
...
%figure('Position',[1 scrsz(4)/1.5 scrsz(3)/1.5 scrsz(4)/1.5])
%1) bin the data
trialNum = size(DeltaF_red,1); binSize = 100;
length_x = plotWin(end)-plotWin(1);
binedF = squeeze(mean(reshape(DeltaF_red(:,1:length_x),trialNum, binSize,[]),2));
imagesc(binedF,[-5 5]);
colormap yellowblue
xlabel('time - water (s)');
h=gca;
...
% save_name = strcat(filename(ii).name,'_aligned');
% save (save_name,'DeltaF','DeltaF_red','Trial_number','Lick_trace','legend_name')
